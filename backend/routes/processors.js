...
router.get("/", async (req, res) => {
  try {
    ...
    const query = url.parse(req.url, true).query.lazyQuery
      ? JSON.parse(url.parse(req.url, true).query.lazyQuery)
...
router.put("/:processorId", async (req, res) => {
  try {
    ...
    if (!data[processorId])
      res
        .status(404)
        .json({ message: "Failed to update data. Proccessor not found!" });
...
router.get("/uniqueEssentials", async (req, res) => {
  try {
    ...
    data.forEach((e) => {
      ...
      if (essentials?.["Product Collection"])
        collection.push(essentials?.["Product Collection"]);
      if (essentials?.["Vertical Segment"])
        segment.push(essentials?.["Vertical Segment"]);
    });
